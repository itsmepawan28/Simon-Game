...
// By clicking the enter btn to start the game
document.addEventListener("keydown", function (event) {
  // Check if Enter is pressed and game hasn't started yet
...
function check() {
  ...
  if (userInput < 2) {
    ...
    alert("Please enter number less than 8 to play easily.");
...
function checkAns(lastBtnIndex) {
  ...
  if (userSeq[lastBtnIndex] === gameSeq[lastBtnIndex]) {
    // for compairing the sequence of each button.

    if (userSeq.length == gameSeq.length) {
      // for level up we compare length.
    ...
    heading.innerText = `Game Over !! Your score was ${level - 1}
                        Refresh to Quit.`;
    // reset();
